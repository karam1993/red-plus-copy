  ...
  <v-dialog v-model="initialvalue.dialog" max-width="350">
    <v-card>
      <v-card-title class="headline">{{
        $t("cards.renewCardTitddle")
      }}</v-card-title>
      <v-card-text>{{ $t("cards.renewCardText") }}
        <v-form ref="form" >
          <v-row>
            <v-col cols="12" class="pb-0">
              ...
              <p v-if="dataForm.packet" class="text-caption error--text">{{ $t('billing.confirmMsg' , {price : dataForm.packet.price + ' ' + dataForm.packet.currency} ) }}</p>
            </v-col>
          </v-row>
        </v-form>
...
export default {
  ...
  created() {
    ...
    this.dataForm.packet = this.initialvalue.oldPacket
  },
  methods: {
    resetConfirm() {
